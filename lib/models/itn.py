...
from SAN.lib import san_utils
from san_utils.image_pool import ImagePool
from san_utils import count_parameters_in_MB
from san_utils import get_model_infos
...
class ITN():
  ...
  def initialize(self, opt, log):
    ...
    san_utils.print_log('------------ Networks initialized -------------', log)
  ...
  def save(self, save_dir, log):
    ...
    san_utils.print_log('save the model into {}'.format(save_dir), log)
  ...
  def load(self, save_dir, log):
    ...
    san_utils.print_log('load the model from {}'.format(save_dir), log)
  ...
  def update_learning_rate(self, log):
    ...
    san_utils.print_log('learning rate = {:.7f}'.format(lr), log)
