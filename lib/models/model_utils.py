...
def roi_pooling(input, rois, size=(7,7)):
  ...
  for i in range(num_rois):
    ...
    im_idx = int(roi[0])
    theta = san_utils.crop2affine(spatial_size, roi[1:])
...
def print_network(net, net_str, log):
  ...
  san_utils.print_log(net, log)
  san_utils.print_log('Total number of parameters for {} is {}'.format(net_str, num_params), log)
