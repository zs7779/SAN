...
from SAN.lib import models
from SAN.lib.datasets.dataset_utils import pil_loader
from SAN.lib.san_vision import transforms
from SAN.lib.datasets.point_meta import Point_Meta
...
class SanLandmarkDetector(object):
    ...
    def detect(self, image, face):
        ...
        if isinstance(image, str) or isinstance(image, Path):
            ...
            raise ValueError('Unsupported input image type {}' % type(image))

        meta = Point_Meta(self.param.num_pts, None, np.array(face[0]), '', 'custom')
